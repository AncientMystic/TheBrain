...
def score_file_by_distance(file_path, centroid):
    ...
    Uses adaptive sampling to avoid full extraction while preserving quality.
    """
    try:
        ...
        if emb is None:
            return float('inf')
        h = exp_map(np.array(emb, dtype=np.float32))
        return hyperbolic_distance(h, centroid)
    except Exception:
        return float('inf')
...
def order_files_by_curriculum(file_paths, max_files=None):
    """Sort files by increasing hyperbolic distance to knowledge centroid."""
    if not file_paths:
        return []
    ...
    if centroid is None:
        ...
        return list(file_paths)  # no prior knowledge, keep original order

    total = len(file_paths)
    print(f"  (Curriculum ordering: scoring {total} files...)")
    scored = []
    for idx, f in enumerate(file_paths, 1):
        if idx % 10 == 0 or idx == total:
            print(f"    Scored {idx}/{total} files")
        d = score_file_by_distance(f, centroid)
        scored.append((d, f))
