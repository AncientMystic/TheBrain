...
def collect_seed_keywords(limit=20):
    """Collect distinct keywords from key_facts and entities tables."""
    ...
    keywords.update(row[0] for row in rows2 if row and row[0])
    return sorted(keywords)


def process_recoll_fast(keyword: str, max_results: int = None, preview_chars: int = None, model: str = None):
    """
    Process Recoll results in batches:
    1. Collect all previews (using snippet/page when available).
    ...
    preview_entries = []   # (pseudo_hash, filepath, original_name, preview_text, chunks)
    seen_hashes = set()
    ...
    for i, res in enumerate(results):
        ...
        try:
            preview = extract_preview(filepath, keyword, page, snippet=snippet)
        except Exception as e:
            print(f"  [{i+1}/{len(results)}] Preview extraction error for {filepath}: {e}")
            continue
        ...
        print(f"  [{i+1}/{len(results)}] Prepared preview for {original_name} ({len(preview)} chars)", flush=True)

    if not preview_entries:
        print("No new previews to process.")
        return

    # Phase 2: Store documents and chunks, generate embeddings
    print("Storing documents and chunks...")
    print(f"  {len(preview_entries)} previews stored.")
    all_previews = [entry[3] for entry in preview_entries]
    doc_hashes = [entry[0] for entry in preview_entries]
    all_chunks_flat = []          # (doc_hash, chunk_idx, chunk_text)
    ...
    print(f"  Processing {len(all_previews)} previews...")
    ...
    # Phase 3: LLM extraction on all chunks, relaxed mode
    print(f"Running knowledge extraction on {len(all_chunk_texts)} chunks...")
    print("  This may take a while for large batches.", flush=True)
    cleaners_mod.RELAXED_MODE = True
    try:
        chunk_results = extract_from_chunks(all_chunk_texts, model=model, chunk_embeddings=chunk_embs, logic_context="")
    finally:
    ...
    for (doc_hash, chunk_idx, chunk_text), chunk_data in zip(all_chunks_flat, chunk_results):
        # Ultra-permissive: keep any fact with non-empty fact_text.
        cleaned_facts = []
        ...
        for fact in cleaned_facts:
            cur_facts.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, 0)
            """, (doc_hash, f"{doc_hash_to_name.get(doc_hash, 'preview')} [preview]",
                  fact.get("fact_type", "other"), fact.get("fact_text", ""),
