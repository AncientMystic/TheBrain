...
# NER sessions stay on CPU even when ONNX_DEVICE=directml: the DML driver
# crashes on concurrent session.run (0xC0000005 observed), which is exactly
# what the threaded pre-pass does. NER models are ms-scale on CPU (no real
# loss); DML keeps accelerating the embedding model where batch throughput
...
# DML sessions have corrupted the heap under multi-session/threaded use
# (observed 0xc0000374); the process-wide onnx_lock plus CPU-gated pools
...
# Coverage-routed cascade: chunks the ONNX pre-pass already covers well skip
# the people/locations/dates LLM category (entities served from pre-pass,
# facts/relationships/events always keep full LLM). Prove with recall diff
# before enabling anywhere near defaults.
...
# Trivium stage-orientation injection (phase 86 wiring). Default ON: the
# stage classifier + DB lookup are proven (smoke-tested 3/3 stages, 4/4
# fallacy detections, zero false positives); disable via TRIVIUM_CONTEXT=false.
TRIVIUM_CONTEXT = os.environ.get("TRIVIUM_CONTEXT", "true").lower() == "true"
...
USE_COREFERENCE_RETRIEVAL = os.environ.get('USE_COREFERENCE_RETRIEVAL', 'true').lower() == 'true'
