...
def repair_mojibake(text: str) -> str:
    """Repair UTF-8 bytes misdecoded as latin-1/cp1252 (e.g. 'â€¯' for U+202F).

    Seen in the wild from typeset PDFs: narrow no-break spaces, non-breaking
    hyphens and dashes surface as â€¯/â€‘/â€". Conservative: only rewrites
    when a latin-1 -> utf-8 round-trip succeeds AND mojibake markers strictly
    decrease. Legitimate â/Ã (French, Portuguese) fail the round-trip or keep
    their markers, so they pass through untouched.
    """
    if not isinstance(text, str) or ("â" not in text and "Ã" not in text):
        return text
    # Sloppy encode: latin-1 bytes straight through, cp1252 specials
    # (€‚ƒ„…†‡ˆ‰Š‹ŒŽ''""•–—˜™š›œžŸ) mapped to 0x80-0x9F. Real-world
    # mojibake mixes both (the middle byte 0x80 usually arrives as €).
    _cp1252 = {0x20AC: 0x80, 0x201A: 0x82, 0x0192: 0x83, 0x201E: 0x84,
    ...
    try:
        ...
        for _ch in text:
            ...
            if _o < 256:
                ...
                _raw.append(_cp1252[_o])
        fixed = bytes(_raw).decode("utf-8")
    except (KeyError, UnicodeDecodeError, ValueError):
        return text

    def _marks(s):
        return s.count("â") + s.count("Ã")
    if _marks(fixed) < _marks(text):
        return fixed
    return text
